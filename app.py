...
def fetch_team_roster(team_abv: str, season: int):
    ...
    for p in roster:
        ...
        ab      = float(stats.get("AB") or stats.get("atBats") or 1)
        hits    = float(stats.get("H")  or stats.get("hits") or 0)
        bb      = float(stats.get("BB") or stats.get("walks") or 0)
        hbp     = float(stats.get("HBP") or 0)
        sf      = float(stats.get("SF") or 0)
        hr      = float(stats.get("HR") or stats.get("homeRuns") or 0)
        doubles = float(stats.get("2B") or stats.get("doubles") or 0)
        triples = float(stats.get("3B") or stats.get("triples") or 0)
        pa  = ab + bb + hbp + sf
        avg = round(hits / ab, 3) if ab > 0 else 0.0
        obp = round((hits + bb + hbp) / pa, 3) if pa > 0 else 0.0
        tb  = hits + doubles + (2 * triples) + (3 * hr)
        slg = round(tb / ab, 3) if ab > 0 else 0.0
        iso = round(slg - avg, 3)
        ops = round(obp + slg, 3)
        wrc_plus = int((ops / 0.730) * 100)
        rows.append({
...
if st.button("Load Today's Slate", type="primary"):
    ...
    with st.status("Fetching games from Tank01...", expanded=True) as status:
        ...
        st.write(f"Found {len(sched)} games. Fetching lineups and rosters...")
        ...
        for _, g in sched.iterrows():
            ...
            away_roster = fetch_team_roster(away_abv, sel_date.year)
            home_roster = fetch_team_roster(home_abv, sel_date.year)
