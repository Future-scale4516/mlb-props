...
def fetch_weather_tank(target_date: str, api_key: str):
    # Tank01 provides pre-game weather data. If unavailable, we fallback to a safe neutral rating.
    ...
    for game_id, details in weather_dict.items():
        ...
        dome = w.get("dome", "false").lower() == "true"
...
def fetch_mlb_historical_stats(season: int):
    # We fetch ALL players once per day to run the heavy math locally
    ...
    for split in splits:
        ...
        rows.append({
            ...
            # Extract simple name for matching with Tank01 (e.g., "Jose Altuve")
...
if st.button("Load Today's Slate", type="primary"):
    ...
    with st.status("Assembling metrics from hybrid data stream...", expanded=True) as status:
        ...
        if sched.empty: st.error("No games found for this date."); st.stop()
        ...
        for _, g in sched.iterrows():
            ...
            a_p_clean = g["away_pitcher"].lower().replace(".", "").replace("'", "")
            h_p_clean = g["home_pitcher"].lower().replace(".", "").replace("'", "")
            ...
            for side_label, roster_df, opp_pitch in [("Away", l_data["away"], home_pitch), ("Home", l_data["home"], away_pitch)]:
                ...
                for _, p_row in roster_df.iterrows():
                    ...
                    b_clean = p_row["name"].lower().replace(".", "").replace("'", "")
