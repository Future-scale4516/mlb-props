...
CALIBRATION_FITS = {
    ...
    "Total Bases": {"base": 0.22, "shrink": 0.30},  # REDUCED from 0.60 — that value was fit against the OLD compound-model output (h-cap=0.95), before the Pozo/Gray 100% bug fix lowered the cap to 0.55 (see p_total_bases_over). That fix alone already substantially lowers every raw TB probability, so applying the old 0.60 shrink on top was double-correcting — found when TB stopped producing any picks at all. 0.30 is a reasoned interim (not a fresh fit) that keeps calibrated output in the range books actually price 2+ TB lines at (~35-45% fair). Needs a proper re-backtest on the current h=0.55 model once a few nights have run, same as any other provisional number in this file.
